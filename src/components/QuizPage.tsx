...
import { ArrowLeft, Award, Lock, CheckCircle, Trophy, Train } from "lucide-react";
...
export function QuizPage({ onNavigate }: QuizPageProps) {
  ...
  const createQuizNodes = (): QuizNode[] => {
    ...
    limitedQuestions.forEach((question, index) => {
      ...
      const positions = [50, 35, 25, 35, 50, 65, 75, 65, 50, 35];
  ...
  if (!showPath && selectedNode) {
    ...
    return (
      <div className="pb-24 px-4 pt-6 max-w-4xl mx-auto min-h-screen bg-gradient-to-b from-[#23272A] to-[#1a1d20]">
        {/* Header */}
        <motion.div
        ...
        >
          <p className="text-gray-400 text-sm mb-1">Station {selectedNode.id + 1} of {limitedQuestions.length}</p>
          <div className="flex justify-center gap-1 mt-2">
            {Array.from({ length: Math.min(10, limitedQuestions.length) }).map((_, i) => (
              <div
                key={i}
                className={`h-1 w-8 rounded-full ${
                  i <= selectedNode.id ? "bg-[#5865F2]" : "bg-[#2C2F33]"
                }`}
              />
            ))}
          </div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <Card className="bg-[#2C2F33] border-[#2F3136] p-6 card-shadow mb-6">
              <div className="mb-6">
                <Badge variant="outline" className="border-[#5865F2] text-[#5865F2] mb-4">
                  {question.pattern}
                </Badge>
                <h2 className="text-white text-xl mb-6">{question.question}</h2>
              </div>
              ...
              {!showFeedback && (
                <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                  <Button
                    ...
                    disabled={question.type === "fill-in" ? !userAnswer : selectedAnswer === null}
          ...
          {showFeedback && (
            ...
            >
              <Card className={`p-6 mb-6 border-2 ${
                ...
                <p className="text-xs text-gray-500 mb-6">Source: {question.source}</p>
  ...
  return (
    <div className="pb-24 px-4 pt-6 min-h-screen bg-gradient-to-b from-[#23272A] to-[#1a1d20]">
      ...
      >
        <h1 className="text-2xl text-white mb-1">🚂 Mediator Pattern Railway</h1>
        <p className="text-gray-400 text-sm">Complete each station to unlock the next</p>
      </motion.div>
      ...
      <div className="relative max-w-md mx-auto">
        ...
        <div 
          ...
          style={{ height: `${quizNodes.length * 160 + 400}px`, top: '100px' }}
        >
          {/* Wooden sleepers */}
          {Array.from({ length: Math.floor((quizNodes.length * 160 + 400) / 20) }).map((_, i) => (
            <div
              key={i}
        ...
        >
          <div className="flex flex-col items-center">
            <div className="w-32 h-32 bg-gradient-to-b from-red-600 to-red-800 rounded-lg border-4 border-red-900 shadow-2xl flex items-center justify-center relative">
              {/* Roof */}
              <div className="absolute -top-8 left-1/2 transform -translate-x-1/2 w-36 h-8 bg-gradient-to-b from-gray-700 to-gray-800 border-t-4 border-gray-900" 
            ...
            <div className="mt-4 bg-[#2C2F33] px-4 py-2 rounded-lg border border-[#2F3136]">
              <p className="text-white font-bold">Start Station</p>
              <p className="text-gray-400 text-xs">Begin your journey</p>
            </div>
          </div>
        </motion.div>

        {/* Quiz Nodes (Trains) */}
        <div className="relative" style={{ height: `${quizNodes.length * 160 + 200}px` }}>
            ...
            return (
              ...
              >
                ...
                <motion.button
                  ...
                  whileHover={node.locked ? {} : { scale: 1.05, y: -5 }}
                ...
                >
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      {/* Icon */}
                      {node.completed ? (
                        <CheckCircle className="w-8 h-8 text-white z-10" fill="white" />
                      ) : node.locked ? (
                  ...
                  <div className="absolute -bottom-10 left-1/2 transform -translate-x-1/2 bg-[#2C2F33] rounded-lg px-3 py-1 border border-[#2F3136]">
                    <span className={`text-xs font-bold ${node.locked ? "text-gray-600" : "text-white"}`}>
                      Station {node.id + 1}
                    </span>
                  </div>
        ...
        {completedNodes.size === limitedQuestions.length && (
          ...
          >
            <div className="w-32 h-32 bg-gradient-to-b from-yellow-400 to-yellow-600 rounded-full flex items-center justify-center border-8 border-yellow-300"
                 style={{ boxShadow: '0 12px 0 #d4a800, 0 15px 30px rgba(0,0,0,0.4)' }}>
              <Trophy className="w-16 h-16 text-white" />
            </div>
            <h2 className="text-2xl text-white mt-8">Journey Complete! 🎉</h2>
            <p className="text-gray-400 mt-2 text-center">You've mastered all stations</p>
            <div className="mt-6 flex items-center gap-2 bg-[#5865F2] px-6 py-3 rounded-2xl glow-effect">
              <Trophy className="w-5 h-5 text-white" />
              <span className="text-white text-lg">{score} XP earned</span>
            </div>
          </motion.div>
        )}
      </div>
